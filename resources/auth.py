...
import jwt, json, bcrypt, uuid, os
...
def token_required(f):
    ...
    def decorated(self, *args, **kwargs):
        ...
        try:
            data = jwt.decode(token, os.getenv('SECRET_KEY'), audience=token_audience, issuer=token_issuer, algorithms=['HS256'])

            claim_sub = data['sub']
            claim_email = data['email']

            user = mongo.db.users.find_one({ '_id': ObjectId(claim_sub), 'email': claim_email })
