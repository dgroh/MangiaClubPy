...
@mock.patch('api.resources.auth.reqparse.current_app', MagicMock())
class TestLoginMethods(unittest.TestCase):
    ...
    @mock.patch('api.resources.auth.make_response')
    @mock.patch('api.resources.auth.jwt')
    def test_post_request_successful(self, jwt_mock, make_response_mock, request_mock):
        with mock.patch('api.resources.auth.app') as app:
            ...
            # Assert
            # TODO: Assert if a real toke is returning and if it has been passed to redis with token sub
